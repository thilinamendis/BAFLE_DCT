...
# Add balanced dataset option to arguments
opt.use_balanced_dataset = False  # Default to False as original implementation works better
opt.use_paired_balanced = True    # New option for paired balanced approach
...
def plot_training_graphs(history, epoch, timestamp):
    """
    Plot and save training metrics graphs
    ...
    # Save combined loss and accuracy plot
    plt.tight_layout()
...
def plot_accuracy_vs_epoch(history, timestamp):
    """
    Create a dedicated plot showing accuracy vs epochs
    ...
    # Ensure y-axis starts at a reasonable value based on the data
    min_acc = min(min(history['train_acc']), min(history['val_acc']))
    plt.ylim([max(0, min_acc-10), 100])
    
    # Add horizontal lines at key accuracy thresholds
    ...
    for i, (train_acc, val_acc) in enumerate(zip(history['train_acc'], history['val_acc'])):
        ...
        if i % 2 == 0:  # Label every other point to avoid crowding
            plt.annotate(f'{train_acc:.1f}', 
                       (epoch_num, train_acc),
                       textcoords="offset points", 
                       xytext=(0,10), 
                       ha='center',
                       fontsize=8)
            plt.annotate(f'{val_acc:.1f}', 
                       (epoch_num, val_acc),
                       textcoords="offset points", 
                       xytext=(0,-15), 
                       ha='center',
    ...
    plt.savefig(f'./plots/accuracy_vs_epoch_{timestamp}.pdf')  # Also save as PDF for publication quality
    plt.close()
...
if __name__ == "__main__":
    ...
    # Log training start and configuration
    ...
    # Load data with appropriate balancing option
    ...
    if not check_point:
        ...
        # If resuming training, load previous history for plotting
    ...
    for epoch in range(START_EPOCH, opt.num_epochs + 1):
        ...
        # Modify training loop to handle different dataset formats
        ...
        # Validation
        # Always validate on the final epoch, otherwise every 2 epochs
        if epoch % 2 == 0 or epoch == opt.num_epochs:
            ...
            # Only save plots at the final epoch
            ...
            state = {
                ...
                # Add history for resuming training
