...
def check_pairwise_filenames(cover_dir, stego_dir, num_pairs=10):
    """
    Check and print the first few cover-stego filename pairs
    
    Args:
        cover_dir: Directory containing cover images
        stego_dir: Directory containing stego images
        num_pairs: Number of pairs to check and print
    """
    cover_files = list(sorted(os.listdir(cover_dir)))
    stego_files = list(sorted(os.listdir(stego_dir)))
    print("\nChecking first few cover-stego pairs:")
    for i in range(min(num_pairs, len(cover_files), len(stego_files))):
        print(f"Pair {i}: Cover = {cover_files[i]} | Stego = {stego_files[i]}")
        if cover_files[i] != stego_files[i]:
            print("  ❌ MISMATCH! These files do not match by name.")
        else:
            print("  ✅ Matched.")

if __name__ == "__main__":
    # Example usage
    cover_dir = "/home/btm0050/Research_AI_Stegno_Analysis/SteganoGan/mount/100bit/100bitdataset/train/original"
    stego_dir = "/home/btm0050/Research_AI_Stegno_Analysis/SteganoGan/mount/100bit/100bitdataset/train/stego"
    ...
    check_pairwise_filenames(cover_dir, stego_dir)
